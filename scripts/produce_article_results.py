...
def main() -> None:
    parser = argparse.ArgumentParser(description='Run the complete AFRL-ES reproduction pipeline.')
    parser.add_argument('--quick', action='store_true', help='Fast verification mode; not article-exact runtime.')
    parser.add_argument('--output-dir', default='outputs/runs/article_reproduction')
    ...
    build_all_charts(summary, out / 'charts', formats=['png', 'pdf', 'svg'])
    print(f'Complete reproduction pipeline finished: {out}')
